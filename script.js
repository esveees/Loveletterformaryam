...
/* ============================================================
   BACKGROUND MUSIC — shared state, callable from any handler
   ============================================================ */
let bgAudio = null;
...
// Call this from directly inside a real user gesture (click/touchend),
// synchronously, with nothing async before it — that's the only way
// mobile browsers reliably allow audio to start.
function tryStartBackgroundMusic() {
  if (!bgAudio || bgStarted || bgStartInFlight || bgUserMuted) return;
...
function initLilyPortal() {
  ...
  const enter = () => {
    if (opened) return;
    opened = true;
    tryStartBackgroundMusic(); // start right here, inside the real tap — don't wait
...
function initBackgroundMusic() {
  ...
  // Try on every real interaction anywhere on the page (lily tap, any
  // button, any scroll-triggered tap) — not gated on the site being
  // visible yet, so it fires on the very first touch instead of missing
  // the gesture window entirely.
  ["pointerdown", "keydown", "touchend", "click"].forEach((evt) => {
    document.addEventListener(evt, tryStartBackgroundMusic);
  });
  document.addEventListener("site:entered", tryStartBackgroundMusic);
