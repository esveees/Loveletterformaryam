...
function initOurSong() {
  ...
  // Keep the visual state tied to the real media state.
  setPlayingUI(!audio.paused);

  btn.addEventListener("click", async () => {
    if (audio.paused) {
      // Stop background music first. If it was playing, remember that so
      // it can be restored when Our Song is paused.
      pauseBackgroundMusic({ remember: true });

      try {
        await audio.play();
      } catch (error) {
        // Autoplay/user-gesture restrictions or a loading error can reject
        // play(). The click itself is already the valid gesture, so leave
        // the UI in the paused state and allow another tap to retry.
        setPlayingUI(false);
        console.warn("Our Song could not be played:", error);
  ...
  audio.addEventListener("play", () => {
    ourSongIsPlaying = true;
    // A successful play event is the authoritative signal that the song is
    // actually playing. This also prevents the two audio tracks overlapping.
    pauseBackgroundMusic();
    setPlayingUI(true);
  });

  audio.addEventListener("pause", () => {
    ourSongIsPlaying = false;
    setPlayingUI(false);
    resumeBackgroundMusicIfNeeded();
  });
  ...
  audio.addEventListener("error", () => {
    ourSongIsPlaying = false;
    setPlayingUI(false);
    console.warn("Our Song audio error:", audio.error);
    resumeBackgroundMusicIfNeeded();
  });
}
...
let bgUserMuted = false;
let bgStartInFlight = false;
let bgStarted = false;
let ourSongIsPlaying = false;

function initBackgroundMusic() {
  bgAudio = document.getElementById("backgroundAudio");

  if (!bgAudio) {
    console.warn("Background audio element #backgroundAudio was not found.");
    return;
  }

  bgAudio.volume = 0.22;
  // Loading is still lazy at the browser's discretion, but preload="auto"
  // avoids an unnecessary delay once the user has interacted with the page.
  bgAudio.preload = "auto";

  const toggle = document.getElementById("musicToggle");

  if (!toggle) {
    console.warn("Background music button #musicToggle was not found.");
    return;
  }
  ...
  const playBackgroundMusic = async () => {
    if (bgUserMuted || ourSongIsPlaying || document.getElementById("site").hidden) {
      return false;
    }
    ...
    if (bgStartInFlight) return false;

    bgStartInFlight = true;
    ...
    try {
      ...
      // Mobile browsers may reject playback until there is a valid user
      // gesture. The next pointer/keyboard/touch interaction will retry.
      bgStarted = false;
      setPressed(false);
      console.warn("Background music could not be played:", error);
      return false;
    } finally {
      bgStartInFlight = false;
    }
  };

  // The lily opens the site from a user interaction, but the later
  // site:entered event is not itself guaranteed to retain that user
  // activation on mobile browsers. Therefore this is only a best-effort
  // start; the real pointer/keyboard/touch listeners below are the fallback.
  document.addEventListener("site:entered", () => {
    playBackgroundMusic();
  });

  const retryAfterUserInteraction = (event) => {
    if (document.getElementById("site").hidden) return;
    if (bgUserMuted || ourSongIsPlaying) return;

    // A real pointer/keyboard/touch event is allowed to satisfy mobile
    // autoplay/user-activation requirements.
    if (event.type === "keydown" && event.repeat) return;
    playBackgroundMusic();
  };

  ["pointerdown", "keydown", "touchend"].forEach((eventName) => {
    document.addEventListener(eventName, retryAfterUserInteraction, { passive: eventName !== "keydown" });
  });

  toggle.addEventListener("click", async () => {
    // Never allow the two tracks to overlap.
    if (ourSongIsPlaying) {
      bgUserMuted = false;
      setPressed(false);
      return;
  ...
  // Keep the button state synchronized with the actual media element.
  ...
  bgAudio.addEventListener("ended", () => {
    ...
    // The element is looped, so this normally should not fire. If a browser
    // reports ended anyway, retry only when it is safe to do so.
    if (!bgUserMuted && !ourSongIsPlaying) {
      playBackgroundMusic();
    }
  });
  ...
  bgAudio.addEventListener("error", () => {
    ...
    bgStartInFlight = false;
    setPressed(false);
    console.warn("Background music audio error:", bgAudio.error);
  });

  setPressed(false);
}

function pauseBackgroundMusic({ remember = false } = {}) {
  if (!bgAudio) return;

  if (remember) {
    bgWasPlayingBeforeSong = !bgAudio.paused && !bgUserMuted;
  }
...
function resumeBackgroundMusicIfNeeded() {
  ...
  const shouldResume = bgWasPlayingBeforeSong && !bgUserMuted && !ourSongIsPlaying;
  bgWasPlayingBeforeSong = false;

  if (shouldResume) {
    // This is normally called from Our Song's pause/ended event, not from a
    // fresh autoplay attempt. If a browser rejects it, a later user gesture
    // will retry the background track.
    bgAudio.play().catch((error) => {
      bgStarted = false;
      const toggle = document.getElementById("musicToggle");
      if (toggle) toggle.setAttribute("aria-pressed", "false");
      console.warn("Background music could not be resumed:", error);
    });
  }
}
